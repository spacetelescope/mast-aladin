...
class MastAladin(Aladin, DelayUntilRendered, AIDA):
    ...
    def add_table(
        ...
        dec_column=None,
        n_sources_max=5_000,
        performance_cls=ConvexHull,
        performance_catalog=None,
        **catalog_options
    ):
        """
        ...
        performance_catalog : instance of `~mast_aladin.catalogs.PerformanceCatalog`,
                              False, None
            Setting to False will prevent any performance catalog from being generated
            automatically.

        performance_cls : class inheriting from `~mast_aladin.catalogs.catalogs.PerformanceCatalog`
            `ConvexHull` or `RandomSubset`
        ...
        **catalog_options : dict
        ...
        # call `Aladin.add_table` if (1) `performance_catalog=False`, (2) the
        # table appears to be an observation query result table (contains an
        # s_region column), (3) n_sources < n_sources_max:
        elif performance_catalog is False or (
                performance_catalog is None and (
    ...
    def _on_viewport_update(self, change):
        ...
        for catalog in self.performance_catalogs:
            catalog.on_viewport_update(change)
