...
class MastAladin(Aladin, DelayUntilRendered, AIDA):
    ...
    """
    performance_layers = List(
        Instance(PerformanceLayer)
    ).tag(sync=True)
    ...
    def add_table(
        ...
        **catalog_options
        ...
        performance_cls : instance of `~mast_aladin.catalogs.catalogs.PerformanceLayer`, None
        ...
        **catalog_options : dict
        ...
        if isinstance(table, PerformanceLayer):
            self.performance_layers.append(table)

        elif (performance_cls is None or
                len(table) < n_sources_max or
                is_likely_an_observation(table)):
            ...
            # otherwise draw a polygon representing the convex hull of all source positions.
            if not isinstance(table, performance_cls):
                ...
                if name is None:
                    name = f"Catalog {len(self.performance_layers) + 1}"
            ...
            self.performance_layers.append(catalog_layer)
    ...
    def _on_viewport_update(self, change):
        ...
        # or the viewport zoom/browser resize/widget aspect ratio (_fov):
        for catalog in self.performance_layers:
